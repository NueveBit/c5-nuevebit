...
<div id="ccm-pagelistPane-add" class="ccm-pagelistPane">
	...
	<div class="ccm-block-field-group">
	  ...
	  <select name="orderBy">
		...
		<option value="most_viewed" <?php  if ($orderBy == 'most_viewed') { ?> selected <?php  } ?>><?php echo t('with the most viewed first')?></option>
		<option value="random" <?php  if ($orderBy == 'random') { ?> selected <?php  } ?>><?php echo t('random order')?></option>
		<option value="relevance" <?php  if ($orderBy == 'relevance') { ?> selected <?php  } ?>><?php echo t('by relevance')?></option>
	  </select>
	</div>
    ...
    <input type="hidden" value="<?=$attribute?>" name="attributes[]" /> 
    <?php endforeach;?>
