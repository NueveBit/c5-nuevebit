 ...
 *
 */
class Concrete5_Controller_Block_NuevebitSlideshow extends BlockController {
	
	protected $btTable = 'nuevebit_btSlideshow';
	...
	protected $btExportTables = array('nuevebit_btSlideshow','nuevebit_btSlideshowItem');
	...
	public function getBlockTypeDescription() {
		return t("Display a running loop of images or videos.");
	}
	
	public function getBlockTypeName() {
		return t("Nuevebit Slideshow");
	}
	...
	function loadItems(){
		...
		$sql = "SELECT * FROM nuevebit_btSlideshowItem WHERE bID=".intval($this->bID).' ORDER BY '.$sortChoices[$this->playback];
		$db = Loader::db();
	...
	function delete(){
		$db = Loader::db();
		$db->query("DELETE FROM nuevebit_btSlideshowItem WHERE bID=".intval($this->bID));		
		parent::delete();
	}
...
//		$this->set('defaultFadeDuration', $this->defaultFadeDuration);
	...
	function duplicate($nbID) {
		...
		$this->loadBlockInformation();
		$db = Loader::db();
		foreach($this->items as $im) {
			$db->Execute('insert into nuevebit_btSlideshowItem (bID, fID, url, type, position, width, height) values (?, ?, ?, ?, ?, ?, ?)', 
				array($nbID, $im['fID'], $im['url'], $im['type'], $im['position'], $im['width'], $im['height'])
			);		
		}
	}
