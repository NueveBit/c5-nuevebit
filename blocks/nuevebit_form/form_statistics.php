 ...
 *
 */
class NuevebitFormStatisticsBlockController {

	public static function getTotalSubmissions($date = null) {
		$db = Loader::db();
		if ($date != null) {
			return $db->GetOne("select count(asID) from nuevebit_btFormAnswerSet where DATE_FORMAT(created, '%Y-%m-%d') = ?", array($date));
		} else {
			return $db->GetOne("select count(asID) from nuevebit_btFormAnswerSet");
		}
	...
	public static function buildAnswerSetsArray( $questionSet, $orderBy='', $limit='' ){
		...
		//get answers sets
		$sql='SELECT * FROM nuevebit_btFormAnswerSet AS aSet '.
		...
		//get answers
		$sql='SELECT * FROM nuevebit_btFormAnswers AS a WHERE a.asID IN ('.join(',',$answerSetIds).')';
